...
@patch("polyplexity_agent.graphs.nodes.researcher.perform_search.get_stream_writer")
...
def test_perform_search_node(
    ...
    url_events = [
        ...
        if len(call[0]) > 0 and isinstance(call[0][0], dict) and call[0][0].get("event") == "web_search_url"
    ]
    assert len(url_events) == 2  # Two URLs in results
